...
_1.2 Identification GoF participants in `Observer, Composite`_
...
-    Component: EventComponent, Composite: EventGroup, Leaf: EventComponent, Client: EventControl
-    EventComponent participates in both the Observer and the Composite patterns acting as a bridge between them. As a ConcreteObserver in the Observer pattern, EventComponent implements the update operation used to receive pushed notifications from the Subject and allows each concrete component to react according to its own behaviour. It receives pushed notifications that it would need to act accordingly on. As a Component in the Composite pattern, EventComponent provides the common interface for both EventGroup and EventComponent and allows the client to treat individual units and groups uniformly.
...
_1.3 UML diagram completion_

![Corrected UML Diagram](figures/_status.png)

![Corrected UML Diagram](figures/_uml.png)

_1.4 Design rationale_

-    (a)A genuine part-whole tree is hierarchial structure where individual objects and groups of objects are organized into a tree. The Component which is the EventComponent is the shared abstract base class for all items. The Leaf which is the EventComponent and the classes inheriting from it are individual objects with no children. A Composite represents a whole made up of Components, while a Leaf represents an individual Component. A Composite can itself be contained within another Composite, allowing the hierarchy to recurse.
...
_2.1 Implementation_

_2.2 Implementation_

_2.3 Object Diagram_

![Object diagram of a composite event component](figures/_task2_object.png)
...
__Task4: Event rules and dynamic behaviour__
...
__Task 5: Sequence diagram portfolio__

_5.1 SD1 - Building and registering part of the event_

![Image showing the sequesnce of events for building and registering part of an event](figures/_task5_sd1.png)

_5.2 SD2 - Cascading event notification_

![Sequence diagram for cascading notification](figures/_task5_sd2.png)

_5.3 SD3 - Conditional event response and Composite behaviour_

![Sequence diagram of Conditional event reposne and Composite behaviour](figures/_task5_sd3.png)

_5.4 SD4 - Your siganature event scenario_

![Sequence diagram showing our siganature event scenario](figures/_task5_sd4.png)

__Task 6: Doxygen documentation__

__Task7 :Git and GitHub Eengineering workflow__

_7.4_

- Work was divided along the practical's four implementation tasks and continued in parallel through active integration. Azolile set up the repository, wrote the Task 1 event concept and architecture, and carried out the largest share of late-stage integration work, including fixing a runtime infinite loop in the Observer notification cycle, resolving null-pointer, dangling-pointer and double-delete issues, correcting a printing bug in `EventComponent::display()` that had been outputting a raw memory address instead of the status string, and completing the Makefile. Patrick implemented the Composite and Observer patterns for Tasks 2 and 3, later restructured the leaf classes to inherit directly from `EventComponent` instead of through the intermediate `EventComponent`, and reworked the Observer attach/detach lifecycle so observers are only notified on `notify()` rather than on attachment. Nicole implemented the Task 4 event rules and dynamic behaviour on a dedicated `feature/task4-dynamic-behaviour` branch, and produced the Task 5 sequence diagrams (SD3 and SD4), which were added to `docs/figures`.

Changes were integrated through `git merge`, and several real conflicts arose and were resolved rather than avoided. `EventControl::admit()` was edited independently on the Task 4 branch and on `dev` at the same time, producing a conflict that Patrick resolved by hand to preserve both the capacity-threshold logic and the Task 4 notification behaviour (commit `96fa59d`). A second conflict, on `EventComponent` itself, was resolved by Azolile during the final integration pass once every task's work had been merged into `dev` (commit `710a542`). Three commits stand out as evidence of genuine collaboration rather than a single end-of-project dump: `96fa59d` and `710a542`, the two conflict resolutions described above, in which two members' independent changes to the same class were reconciled by hand; and `c06dc25`, in which Azolile fixed the Observer notify infinite loop discovered only after Task 4's dynamic behaviour was exercising the merged Composite/Observer code at runtime, showing the team debugging shared code together after integration rather than treating each member's merge as final.

We did not use formal GitHub Pull Requests for this practical; branches (`dev` and `feature/task4-dynamic-behaviour`) were merged directly once reviewed between members over calls/emails, and coordination was tracked through commit messages and branch names rather than a PR review thread. The repository history spans 23–28 August 2026 with commits from all three members throughout, rather than a single late submission.

__Task 8: Intergration and demonstration__
